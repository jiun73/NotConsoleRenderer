...
ntp::ClientPtr ntp_client_global = nullptr;
...
size_t get_clock()
{
	if (ntp_client_global != nullptr)
	{
		if (ntp_client_global->clock() != nullptr)
		{
			return std::chrono::duration_cast<std::chrono::milliseconds>(ntp_client_global->clock()->now().time().time_since_epoch()).count();
		}
	}
...
void test_clients(std::list<std::thread>& threads)
{
	...
	//threads.push_back(std::thread(get_clock));
}
...
void test_clients_manager(ntp::IManager::ManagerSettings::ServersEvaluationMethod evalMthd, std::list<std::thread>& threads)
{
	...
	;
	threads.push_back(std::thread(execute_ntp_client_query_loop, mng->active_server().first->server_ep));
	//threads.push_back(std::thread(get_clock));
}
