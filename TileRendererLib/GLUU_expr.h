...
#define GLUU_Make(_args, _name) static string keyword() { return _name; } std::type_index type() override { return typeid(decltype(*this)); } pair<size_t, string> fetch_keyword() override {return { _args, keyword() };} shared_ptr<::GLUU::Widget> make(vector<string_ranges>& args, ::GLUU::Parser& parser) override
