...
#define __REGISTER_ENTITYX_SYSTEM__(layer, type) inline ::ECSX::QuickSystemXAdder<layer, type> MACRO_CONCAT(system_adder, __COUNTER__)
	...
	template<size_t L, typename T, typename F>
	struct QuickSystemManagerXAdderHelper;

	template<size_t L, typename T, typename R, typename... Ts>
	struct QuickSystemManagerXAdderHelper<L, T, R(T::*)(vector<Ts*>&..., vector<EntityID>&)> : public SystemXManagerAdder<L, T, Ts...> {};

	template<size_t L, typename T>
	struct QuickSystemManagerXAdder : public QuickSystemManagerXAdderHelper<L, T, decltype(&T::update)> {};

#define CONCAT_IMPL( x, y ) x##y
#define MACRO_CONCAT( x, y ) CONCAT_IMPL( x, y )
#define __REGISTER_ENTITYX_SYSTEM_MANAGER__(layer, type) inline ::ECSX::QuickSystemManagerXAdder<layer, type> MACRO_CONCAT(system_adder, __COUNTER__)
